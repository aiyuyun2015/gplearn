...
def check_transformer1():
    ...
    gp = SymbolicTransformer(generations=20, population_size=2000,
                             ...
                             max_samples=0.9,
                             random_state=0)
    tmp_x = diabetes.data[:300, :]
    tmp_y = diabetes.target[:300]
    gp.fit(tmp_x, tmp_y)

    print("[Important] factor=", gp._best_programs[0].__str__())
    dot_data = gp._best_programs[0].export_graphviz()
    # Create a Source object
    source = Source(dot_data)

    # Render the DOT data to an image file (e.g., 'program.png')
    source.render('program', format='png', cleanup=True)  # This saves 'program.png'

    # Re-run model with new factor
